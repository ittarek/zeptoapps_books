...
// const ReactImageFallback = React.lazy(() => import("react-image-fallback"));

const BookData = ({ books, loading, error, currentPage, totalPages, setCurrentPage }) => {
    ...
    <Container>
      <div className="book-container flex justify-between  mt-[12%] lg:mt-[7%]">
        <div className="list-items lg:w-[50%]">
          {/* Pagination */}
          <div className=" fixed top-0 lg:top-[85%]  lg:bottom-0  left-[35%] mx-auto mt-2 lg:mt-0 lg:mb-11 lg:px-5 lg:py-3 bg-[#C5DFFF] rounded-full w-[35%] flex justify-between items-center font-bold border border-[#000] z-10">
            <button
        ...
        <div className="max-w-sm details_card overflow-hidden shadow-lg w-[50%] lg:w-[40%] border mr-11  fixed right-0  hover:bg-[#C5DFFF] duration-500  hover:scale-105">
          ...
          <div className="data-details p-1 overflow-hidden px-6 py-4">
            ...
            <div className="font-bold text-xl my-2">{showDetails.title}</div>
            {showDetails ? (
              <p className="text-gray-700 text-base">{showDetails.subjects}</p>
            ) : (
              <Skeleton />
            )}
